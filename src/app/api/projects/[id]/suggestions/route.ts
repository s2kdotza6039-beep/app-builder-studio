...
export async function GET(
  ...
  try {
    ...
    const appNameSafe = project.app_name || "Custom Application";
    const appTypeSafe = project.app_type || "SaaS Platform";

    const apiKey = process.env.OPENAI_API_KEY;
    if (USE_MOCK || !apiKey) {
      return NextResponse.json({
        success: true,
        suggestions: getFallbackSuggestions(appNameSafe, appTypeSafe),
      });
    }
    ...
    const userMessage = [
      ...
      `App Name: "${appNameSafe}"`,
      `App Type: "${appTypeSafe}"`,

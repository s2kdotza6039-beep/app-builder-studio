...
export default function ForgeWorkspace({
  ...
  // Check for GitHub OAuth callback result in URL
  useEffect(() => {
    ...
    if (githubStatus === "connected") {
      ...
      setNotice({
        type: "success",
        text: "GitHub connected! You can now push your code.",
      });
      ...
      setNotice({
        type: "error",
        text: "GitHub connection failed or was denied.",
      });
      window.history.replaceState({}, "", window.location.pathname);
    }
  }, [projectId]);
  ...
  async function handleGenerate() {
    ...
    try {
      ...
      setNotice({
        type: "success",
        text: `Generated ${data.filesGenerated ?? data.files?.length} files.`,
      });
  ...
  async function handleGitHubPush() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setNotice({
          type: "success",
          text: `✅ Pushed ${data.filespushed} files to GitHub! ${
            data.isNew ? "New repo created." : "Repo updated."
          }`,
        });
      } else {
    ...
    try {
      ...
      if (data.updatedFiles?.length > 0) {
        ...
        setShangMessages((prev) => [
          ...
          {
            role: "assistant",
            message: `✅ ${data.filesChanged} file${
              data.filesChanged === 1 ? "" : "s"
            } updated. Preview refreshed. Version saved.`,
          },
        ]);
      }
    } catch {
    ...
    <div className="flex h-full min-h-0 relative">
      ...
      {!shangCollapsed ? (
          ...
          <div className="min-h-0 flex-1 overflow-y-auto p-3 space-y-3">
            {shangMessages.map((msg, i) => (
              ...
              >
                ...
                >
                  {msg.role === "assistant" && (
                    <p className="text-xs text-orange-400 font-black mb-1">
                      🥋 Shang Tsung
                    </p>
                  )}
                  {msg.message}
          ...
          <div className="px-3 pb-2">
            ...
            <div className="flex flex-wrap gap-1.5">
              {visibleChips.map((chip, i) => {
                const usageCount =
                  habits.session.chipUsage[chip.message]?.count || 0;
                return (
                  <button
                    key={i}
        ...
        <div className="flex flex-col items-center border-r border-stone-800 bg-stone-900 w-10 shrink-0">
          ...
          >
            ...
            <span className="text-stone-600 group-hover:text-orange-400 transition text-xs">
              ▶
            </span>
          </button>
        </div>
      ...
      {!explorerCollapsed ? (
          ...
          <div className="min-h-0 flex-1 overflow-y-auto p-2">
            {!hasFiles ? (
              ...
              <ul className="space-y-0.5">
                {files.map((f) => {
                  const openCount =
                    habits.session.fileHistory[f.file_path]?.openCount || 0;
                  return (
                    <li key={f.id}>
                      <button
                        type="button"
        ...
        <div className="flex flex-col items-center border-r border-stone-800 bg-stone-900 w-10 shrink-0">
          ...
          >
            <span className="text-base">📁</span>
            <span className="text-stone-600 group-hover:text-stone-100 transition text-xs">
              ▶
            </span>
          </button>
        </div>
      ...
      <section className="flex flex-1 min-w-0 flex-col bg-stone-950">
        ...
        <div className="flex h-11 items-center justify-between border-b border-stone-800 bg-stone-900 px-4 shrink-0">
          ...
          <div className="flex items-center gap-2">
            ...
            {/* File path + copy (code view only) */}
            {viewMode === "code" && selectedFile && (
              <>
                <span className="truncate font-mono text-xs text-stone-500 max-w-[120px]">
            ...
            {hasFiles && (
              <button
                ...
                className={`border px-2.5 py-1.5 rounded-lg text-xs font-black transition disabled:opacity-50 ${
                  githubConnected
                    ? "border-emerald-800/50 bg-emerald-900/20 hover:bg-emerald-900/30 text-emerald-400 hover:text-emerald-300"
                    : "border-stone-700 hover:border-stone-500 bg-stone-900 hover:bg-stone-800 text-stone-400 hover:text-stone-100"
                }`}
                title={
                  githubConnected
                    ? `Push to GitHub (${githubUsername})`

...
export async function POST(request: Request) {
  try {
    ...
    const { analysis, destination } = await request.json();

    if (!analysis) {
      return NextResponse.json({ error: "No analysis data provided" }, { status: 400 });
    }

    // Create the project with detected planning data
    const project = await prisma.project.create({
      data: {
        user_id: user.id,
        app_name: analysis.app_name,
        app_description: analysis.app_description,
        ...
        routes: {
          create: (analysis.detected_routes || []).map(
            (r: { page_name: string; route_path: string; purpose: string }, i: number) => ({
    ...
    // If going to Forge, save the uploaded files as ProjectFiles
    if (destination === "forge" && analysis.files?.length > 0) {
      // Limit to 50 files max for the forge
      const filesToSave = analysis.files.slice(0, 50);

      await Promise.all(
        filesToSave.map((f: { file_path: string; content: string; language: string }) =>
      ...
      await prisma.projectVersion.create({
        data: {
          ...
          files_snapshot: filesToSave.map((f: { file_path: string; content: string; language: string }) => ({
            file_path: f.file_path,
            content: f.content,
            language: f.language,
          })),
